...
# the last distance in the distances array is equal to the distance from the
# last city to the first city. You'll also be given an array of fuel available
# at each city, where fuel[i] is equal to the fuel available at city i. The
# total amount of fuel available (from all cities combined) is exactly enough to
# travel to all cities. Your fuel tank always starts out empty, and you're given
# a positive integer value for the number of miles that your car can travel per
# gallon of fuel (miles per gallon, or MPG). You can assume that you will always
...
# Inefficient O(n^2) time solution
...
def validStartingCity(distances, fuel, mpg):
  for i in range(len(distances)):
    ...
    for j in range(len(distances)):
      city_index = (i + j) % len(distances)
      current_fuel += (fuel[city_index] * mpg)
      current_fuel -= distances[city_index]
      if current_fuel < 0:
